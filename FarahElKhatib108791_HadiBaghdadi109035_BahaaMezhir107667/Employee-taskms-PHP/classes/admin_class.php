...
/* ---------------------- Admin Login Check ----------------------------------- */
    ...
    public function admin_login_check($data) {
        ...
        try {
            $sql = "SELECT IF(check_login(:username, :password) > 0, 1, 0) as valid";
            ...
            if($result['valid']) {
                // Get user details for valid login
		...
		try{
			...
			if($total_email != 0 && $total_username != 0){
				...
				if(isset($data['skills']) && is_array($data['skills'])) {
					$add_skill = $this->db->prepare("INSERT INTO employee_skills (user_id, skill_name, proficiency_level) VALUES (:user_id, :skill, :level)");
					
					foreach($data['skills'] as $index => $skill) {
						if(!empty($skill)) {
							$proficiency = $data['proficiency'][$index];
							$add_skill->bindParam(':user_id', $user_id);
							$add_skill->bindParam(':skill', $skill);
							$add_skill->bindParam(':level', $proficiency);
							$add_skill->execute();
						}
					}
				}
...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		$required_skill = $this->test_form_input_data($data['required_skill']);

		try{
			$add_task = $this->db->prepare("INSERT INTO task_info (t_title, t_description, required_skill, t_start_time, t_end_time, t_user_id) 
										  VALUES (:x, :y, :z, :a, :b, :c)");

			$add_task->bindparam(':x', $task_title);
			$add_task->bindparam(':y', $task_description);
			$add_task->bindparam(':z', $required_skill);
			$add_task->bindparam(':a', $t_start_time);
			$add_task->bindparam(':b', $t_end_time);
			$add_task->bindparam(':c', $assign_to);
			...
			try{
				$update_task = $this->db->prepare("UPDATE task_info SET 
					...
					assignment_status = CASE 
						WHEN t_user_id != :b2 THEN 'pending'
				...
				$update_task->bindparam(':b2', $assign_to);
...
// Add method to get employee skills
public function get_employee_skills($user_id) {
    $sql = "SELECT * FROM employee_skills WHERE user_id = :user_id";
    $stmt = $this->db->prepare($sql);
